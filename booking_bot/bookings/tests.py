...
class BookingAdminTests(TestCase):
    def setUp(self):
        ...
        UserProfile.objects.create(
            user=self.owner_user, role="admin"
        )  # Property owner must be 'admin' role based on model limit_choices_to
        ...
        self.booking_user = User.objects.create_user(
            username="booker", password="password"
        )
    ...
    def test_save_model_calculates_total_price(self):
        ...
        self.booking_admin.save_model(
            request, booking_instance, form, change=False
        )  # change=False for new object
