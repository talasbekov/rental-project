...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=30,
            help='Количество дней для отчета'
        )
        parser.add_argument(
            '--user',
            type=str,
            help='Фильтр по пользователю'
        )
    ...
    def handle(self, *args, **options):
        ...
        suspicious = logs.filter(
            action__in=['view_code', 'send_code', 'export_data']
        ).values('user__username').annotate(
            count=Count('id')
        ).filter(count__gt=50).order_by('-count')

        if suspicious:
            self.stdout.write(self.style.WARNING("\n⚠️ Подозрительная активность:"))
            for stat in suspicious:
                self.stdout.write(f"  {stat['user__username']}: {stat['count']} доступов к конфиденциальным данным")
