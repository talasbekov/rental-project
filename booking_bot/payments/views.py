...
from rest_framework.permissions import (
    IsAuthenticated,
)  # Or custom permission for bot/service
...
from .kaspi_service import (
    initiate_payment as kaspi_initiate_payment_service,
    KaspiPaymentError,
)
...
def kaspi_payment_webhook(request):
    ...
    if request.method == "POST":
        ...
        if not kaspi_payment_id:
            ...
            return JsonResponse(
                {"status": "error", "message": "payment_id is required"}, status=400
            )
        ...
        try:
            ...
            if not booking:
                logger.error(
                    f"Kaspi webhook: Бронирование не найдено для payment_id: {kaspi_payment_id}"
                )
                return JsonResponse(
            ...
            if not new_status:
                ...
                return JsonResponse(
                    {"status": "success", "message": "Status not processed"}, status=200
                )
            ...
            if booking.status != new_status:
                ...
                if new_status == "confirmed" and old_status != "confirmed":
                    try:
                        ...
                        if user_profile.telegram_chat_id:
                            send_telegram_booking_confirmation(
                                user_profile.telegram_chat_id, booking
                            )
                            logger.info(
                                f"Отправлено подтверждение в Telegram для бронирования {booking.id}"
                            )

                        # Отправляем уведомление в WhatsApp
                        if user_profile.phone_number:
                            send_whatsapp_booking_confirmation(
                                user_profile.phone_number, booking
                            )
                            logger.info(
                                f"Отправлено подтверждение в WhatsApp для бронирования {booking.id}"
                            )
                        ...
                        logger.error(
                            f"UserProfile не найден для пользователя {booking.user.id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Ошибка при отправке уведомлений для бронирования {booking.id}: {e}"
                        )
                ...
                # Обработка неудачной оплаты
                    ...
                    try:
                        ...
                        if user_profile.telegram_chat_id:
                            send_telegram_payment_error(
                                user_profile.telegram_chat_id, booking, error_message
                            )

                        if user_profile.phone_number:
                            send_whatsapp_payment_error(
                                user_profile.phone_number, booking, error_message
                            )

                    except Exception as e:
                        logger.error(
                            f"Ошибка при отправке уведомления об ошибке платежа: {e}"
                        )

            else:
                logger.info(
                    f"Статус бронирования {booking.id} не изменился: {booking.status}"
                )
...
class KaspiInitiatePaymentView(APIView):
    permission_classes = [
        IsAuthenticated
    ]  # Ensure only authenticated users/services can call this
    ...
    def post(self, request, *args, **kwargs):
        ...
        if booking.status not in [
            ...
            return Response(
                {
                    "error": f'Booking status is "{booking.get_status_display()}", cannot initiate payment.'
                },
        ...
        try:
            ...
            if (
                payment_info
                and payment_info.get("checkout_url")
                and payment_info.get("payment_id")
            ):
                ...
                logger.info(
                    f"Kaspi payment initiated for Booking ID {booking.id}. Kaspi ID: {payment_info['payment_id']}"
                )
                return Response(
                    {
                ...
                logger.error(
                    f"Kaspi service failed to return checkout_url or payment_id for Booking ID {booking.id}. Response: {payment_info}"
                )
                # Optionally set booking status to payment_failed here if appropriate
                # booking.status = 'payment_failed'
                # booking.save()
                return Response(
                    {"error": "Payment initiation failed with Kaspi service."},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )

        except KaspiPaymentError as e:
            logger.error(
                f"KaspiPaymentError for Booking ID {booking.id}: {e}", exc_info=True
            )
            booking.status = "payment_failed"  # Set status on error from Kaspi
            booking.save()
            return Response(
                {"error": f"Kaspi payment error: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        except Exception as e:
            logger.error(
                f"Unexpected error initiating payment for Booking ID {booking.id}: {e}",
                exc_info=True,
            )
