...
from .utils import (
    ...
    send_whatsapp_list_message, send_whatsapp_document,
...
def handle_add_property_start(phone_number: str, text: str) -> Optional[bool]:
    ...
    if state == STATE_ADMIN_ADD_PROPERTY:
        ...
        send_whatsapp_message(
            phone_number,
            "Шаг 2/10: Введите *описание* квартиры:"
        )
        return True
    ...
    if state == STATE_ADMIN_ADD_DESC:
        ...
        send_whatsapp_message(
            phone_number,
            "Шаг 3/10: Введите *адрес* квартиры:"
        )
        return True
    ...
    if state == STATE_ADMIN_ADD_CITY:
        try:
            ...
            send_whatsapp_message(phone_number, "❌ Город не найден. Попробуйте ещё раз.")
        return True
    ...
    if state == STATE_ADMIN_ADD_DISTRICT:
        try:
            district = District.objects.get(
                name=text,
                city_id=state_data['new_property']['city_id']
            )
            ...
            send_whatsapp_button_message(
                phone_number,
                "Шаг 6/10: Выберите *класс* жилья:",
                buttons
            )
        except District.DoesNotExist:
            send_whatsapp_message(phone_number, "❌ Район не найден. Попробуйте ещё раз.")
        return True
    ...
    if state == STATE_ADMIN_ADD_CLASS:
        ...
        if text in mapping:
            ...
            send_whatsapp_list_message(
                phone_number,
                "Шаг 7/10: Сколько *комнат*?",
                "Выбрать",
                sections
            )
        else:
            send_whatsapp_message(phone_number, "❌ Неверный выбор. Попробуйте ещё раз.")
        return True
    ...
    if state == STATE_ADMIN_ADD_ROOMS:
        try:
            ...
            send_whatsapp_message(
                phone_number,
                "Шаг 8/10: Введите *площадь* (м²):"
            )
        except ValueError:
            send_whatsapp_message(
                phone_number,
                "❌ Неверный формат. Введите количество комнат."
            )
        return True
    ...
    if state == STATE_ADMIN_ADD_AREA:
        try:
            ...
            send_whatsapp_message(
                phone_number,
                "Шаг 9/10: Введите *цену* за сутки (₸):"
            )
        except ValueError:
            send_whatsapp_message(
                phone_number,
                "❌ Неверный формат площади. Введите число."
            )
        return True
    ...
    if state == STATE_ADMIN_ADD_PRICE:
        try:
            ...
            send_whatsapp_message(phone_number, "❌ Неверный формат цены. Введите число.")
        except Exception as e:
            logger.error(f"Error creating property: {e}", exc_info=True)
            send_whatsapp_message(phone_number, "❌ Ошибка при сохранении. Попробуйте снова.")
        return True
    ...
    if state == STATE_ADMIN_ADD_PHOTOS:
        ...
        if not prop_id:
            send_whatsapp_message(
                phone_number,
                "❌ Не удалось найти созданную квартиру."
            )
            profile.whatsapp_state = {}
            profile.save()
            return True
        ...
        if photo_mode is None:
            if text == "URL фото":
                ...
                send_whatsapp_message(
                    phone_number,
                    f"✅ Квартира создана без фотографий!"
                )
                profile.whatsapp_state = {}
                profile.save()
                ...
                send_whatsapp_message(
                    phone_number,
                    "Пожалуйста, выберите способ загрузки фотографий."
                )
            return True
        ...
        if photo_mode == 'url' and text:
            ...
            if created > 0:
                ...
                send_whatsapp_message(
                    phone_number,
                    "❌ Не удалось добавить фотографии. Проверьте URL."
                )
            return True
        ...
        if photo_mode == 'device':
            send_whatsapp_message(
                phone_number,
                "Пожалуйста, отправьте фотографии как изображения."
            )
            return True
...
def show_extended_statistics(phone_number, period='month'):
    ...
    send_whatsapp_button_message(
        phone_number,
        "Выберите период или скачайте отчет:",
        buttons
    )
...
def export_statistics_csv(phone_number, period='month'):
    ...
    writer.writerow([
        'ID', 'Квартира', 'Пользователь', 'Заезд', 'Выезд',
        'Цена', 'Статус', 'Создано'
    ])
