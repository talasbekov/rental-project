...
from booking_bot.users.models import (
    UserProfile,
)  # To get phone number if only user is available
...
def send_booking_confirmation_notification(booking_id):
    ...
    try:
        booking = Booking.objects.select_related(
            "user", "property", "user__profile"
        ).get(id=booking_id)
        user_profile = booking.user.profile

        if not user_profile.phone_number:
            logger.warning(
                f"User {booking.user.username} has no phone number in profile for booking {booking_id}."
            )
            return
        ...
        send_whatsapp_message(
            user_profile.phone_number.replace("whatsapp:", ""), message_body
        )
        logger.info(
            f"Sent booking confirmation for booking ID {booking_id} to {user_profile.phone_number}"
        )
        ...
        logger.error(
            f"Cannot send confirmation: UserProfile for user {booking.user.username} not found for booking {booking_id}."
        )
    except Exception as e:
        logger.error(
            f"Error sending booking confirmation for {booking_id}: {e}", exc_info=True
        )
...
def send_payment_failed_notification(booking_id, reason=""):
    ...
    try:
        ...
        logger.info(
            f"Sent payment failed notification for booking ID {booking_id} to {user_profile.phone_number}"
        )
    except Booking.DoesNotExist:
        logger.error(
            f"Cannot send payment failed notification: Booking ID {booking_id} not found."
        )
    except UserProfile.DoesNotExist:
        logger.error(
            f"Cannot send payment failed notification: UserProfile for user {booking.user.username} not found for booking {booking_id}."
        )
    except Exception as e:
        logger.error(
            f"Error sending payment failed notification for {booking_id}: {e}",
            exc_info=True,
        )
...
def send_booking_cancelled_notification(booking_id, cancelled_by_user=True):
    ...
    try:
        ...
        logger.info(
            f"Sent booking cancellation notification for booking ID {booking_id} to {user_profile.phone_number}"
        )
    except Booking.DoesNotExist:
        logger.error(
            f"Cannot send cancellation notification: Booking ID {booking_id} not found."
        )
    except UserProfile.DoesNotExist:
        logger.error(
            f"Cannot send cancellation notification: UserProfile for user {booking.user.username} not found for booking {booking_id}."
        )
    except Exception as e:
        logger.error(
            f"Error sending booking cancellation for {booking_id}: {e}", exc_info=True
        )
