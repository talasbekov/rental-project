...
class WhatsAppHandlerTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_user(username='propowner', password='testpassword')
        UserProfile.objects.create(user=self.admin_user, role='admin', phone_number='+111000111')

        self.property1 = Property.objects.create(
            name="Cosy Apartment", owner=self.admin_user, price_per_day=50.00,
            number_of_rooms=1, property_class='economy', address="1 Main St", area=30
        )
        self.property2 = Property.objects.create(
            name="Luxury Villa", owner=self.admin_user, price_per_day=200.00,
            number_of_rooms=4, property_class='luxury', address="2 Ocean Drive", area=150
        )
    ...
    def test_handle_unknown_user_registers_new_user(self):
        ...
        self.assertTrue(User.objects.filter(username=f"user_{self.test_user_phone.replace('+', '')}").exists())
        user = User.objects.get(username=f"user_{self.test_user_phone.replace('+', '')}")
        self.assertTrue(UserProfile.objects.filter(user=user, phone_number=self.test_user_phone, role='user').exists())
    ...
    def test_handle_known_user_book_command_success(self):
        ...
        self.assertTrue(Booking.objects.filter(user=user_profile.user, property=self.property1).exists())
    ...
    def test_handle_known_user_book_command_property_not_found(self):
        ...
        self.assertFalse(Booking.objects.filter(user=user_profile.user).exists()) # No booking created
        mock_book_response.message.assert_called_once()
        self.assertIn("Property with ID 9999 not found.", mock_book_response.message.call_args[0][0])
    ...
    def test_handle_known_user_book_command_unavailable(self):
        ...
        mock_book_response.message.assert_called_once()
        self.assertIn(f"Sorry, {self.property1.name} is not available for the selected dates.", mock_book_response.message.call_args[0][0])
