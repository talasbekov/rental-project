...
from booking_bot.whatsapp_bot.handlers import (
    send_welcome_message, get_user_state, set_user_state, # Added set_user_state
    handle_known_user, ACTION_MAIN_MENU, ACTION_SELECTING_REGION,
    BUTTON_SEARCH_APARTMENTS, REGIONS, # _send_message_with_buttons (private, usually tested via public methods)
)
...
class HandlersWelcomeTest(TestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(user=self.user, phone_number='+1234567890', whatsapp_state={})
    ...
    def test_send_welcome_message(self):
        ...
        self.assertIn(BUTTON_SEARCH_APARTMENTS, response_str) # Check if button text is in the message
...
class HandlersRegionSelectionTest(TestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(user=self.user, phone_number='+1234567892')
    ...
    def test_handle_known_user_clicks_search_apartments(self, mock_send_region_selection):
        response_twiml = MessagingResponse()
        ...
        handle_known_user(self.user_profile, "1", response_twiml) # "1" should map to the button

        mock_send_region_selection.assert_called_once_with(self.user_profile, response_twiml)

        # To test the state change by send_region_selection, you'd test send_region_selection directly:

    @patch('booking_bot.whatsapp_bot.handlers._send_message_with_buttons') # Mock the actual message sending
    def test_send_region_selection_sets_state_and_sends_message(self, mock_send_msg_with_buttons):
        response_twiml = MagicMock(spec=MessagingResponse) # Use MagicMock for TwiML

        # Call the function directly
        from booking_bot.whatsapp_bot.handlers import send_region_selection # Local import for clarity
        ...
        self.assertEqual(args[0], response_twiml) # First arg is twilio_messaging_response
...
class HandlersRoomSelectionTest(TestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(user=self.user, phone_number='+1234567893')
